...
void LL1::follow(const formula &form) {
 ...
 //
 void LL1::analyze(QtGuiApplication2 &win) {
	 ...
	 table->setHorizontalHeaderItem(0, new QStandardItem(QObject::tr("step")));
	 table->setHorizontalHeaderItem(1, new QStandardItem(QObject::tr("grammer_stack")));
	 table->setHorizontalHeaderItem(2, new QStandardItem(QObject::tr("in_words")));
	 table->setHorizontalHeaderItem(3, new QStandardItem(QObject::tr("produce")));

	 win.ui.tableV->setModel(table);

	 while (*m_it!='#') {

		 table->setItem(step, 0, new QStandardItem(QString::fromStdString(string(1, ('0'+step)))));
		// ////cout <<setfill(' ')<< resetiosflags(ios::right) << setw(10);
		 string m_str;
		 for (auto m : mark_stack) {
			 m_str+=string(1,m);
		 }
		 m_str += string(1, '\0');
		 table->setItem(step, 1, new QStandardItem(QString::fromStdString(m_str)));
	...
	//	 ////cout << setfill(' ') << setw(20);
		// ////cout.width(16);
		 ...
		 for (int i = in_words.size() - 1; i > 0; i--) {
			 w_str += string(1, in_words[i]);
		 }
		 w_str += string(1, '\0');
		 table->setItem(step, 2, new QStandardItem(QString::fromStdString(w_str)));

		// ////cout << "\t ";
		 //////cout << "\t\t\t";
	//	 ////cout << setfill(' ') << resetiosflags(ios::left) << setw(10);		//??如何对齐？？
		 if (trans) {
			 trans = false;	
		//	 ////cout.width(16);
			 table->setItem(step, 3, new QStandardItem(QString::fromStdString( next_pro)));
		 }
		 step++;
 ...
 void LL1::run(QtGuiApplication2 &win){
	 ...
	 for (auto str:raw_produces) {
		...
		// //cout << fo.isEntire << endl;
		 win.ui.textBrowser_3->append(QString::fromStdString(fo.produce));
		G_add(fo);
		 //if (!G_add(formula(*it))) {
