...
private:
	...
	string showStrStack();		//显示输入栈
	string showStatusStack();
	string showParseStack();
...
public:
	...
	void build(Qt3 &win);		//构造Action.goto表
	void showTable(Qt3 &win);	//打印LR分析表
						// debug();
	void loadStr(const string &str);		//读取输入串
	void analyze(Qt3 &win);		//LR(1)分析
	void showGrammer();	//显示输入的文法产生式
						//void drawGrapgh();		//画出DFA
						//void generateDot();		//
	void run(const string& str, Qt3 &win);
};
